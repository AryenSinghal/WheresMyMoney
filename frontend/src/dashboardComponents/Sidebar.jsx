    ...
    <>
      ...
      <button
        ...
        className="md:hidden fixed top-4 left-4 z-50 bg-white/5 backdrop-blur-md
          transition-all duration-200 ease-in-out border border-white/10 rounded-xl shadow-lg text-white p-3 hover:bg-white/10"
      >
          ...
          <ul className="space-y-3">
            <li>
              <Link
                ...
                onClick={() => setIsOpen(false)}
              >
                <button className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 ${isActive('/')}`}>
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <li>
              <Link
                ...
                onClick={() => setIsOpen(false)}
              >
                <button className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all duration-200 ${isActive('/receipts')}`}>
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <li className="relative pt-4">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-3 rounded-xl bg-purple-500 hover:bg-purple-600 transition-all duration-200"
              ...
              {isDropdownOpen && (
                <div className="absolute top-full left-0 right-0 bg-gray-900/95 backdrop-blur-sm text-white rounded-xl shadow-xl mt-2 overflow-hidden z-60">
                  <ul className="divide-y divide-white/10">
                    ...
                    <li>
                      <button onClick={openCamera} className="flex items-center gap-3 p-4 w-full hover:bg-white/10">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="hidden md:flex items-center justify-between fixed top-0 left-0 right-0 h-16 text-white p-4 z-50 backdrop-blur-xl bg-black/30 border-b border-white/10">
        ...
        <ul className="flex items-center gap-2">
          <li>
            <Link to="/" className="hover:text-white">
              <button className={`px-4 py-2 rounded-xl transition-all duration-200 ${isActive('/')}`}>
                My Dashboard
              </button>
            </Link>
          </li>
          <li>
            <Link to="/receipts" className="hover:text-white">
              <button className={`px-4 py-2 rounded-xl transition-all duration-200 ${isActive('/receipts')}`}>
                Receipts
              </button>
            </Link>
          </li>
        ...
        <div className="relative">
          <button
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-xl transition-all duration-200 flex items-center gap-2"
            disabled={isLoading || isEmailFetching}
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
